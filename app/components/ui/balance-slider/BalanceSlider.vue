<template>
  <div class="slider-container relative mx-auto my-0 grid place-items-center overflow-hidden" :style="sliderStyles" @mouseenter="active = 1;" @mouseleave="active = 0;" @focusin="active = 1;" @focusout="active = 0;" @touchstart="active = 1;" @touchend="active = 0;">
    <input id="track" v-model="value" type="range" min="0" max="100" class="size-full touch-none opacity-0 hover:cursor-grab focus-visible:outline-offset-4 focus-visible:outline-transparent active:cursor-grabbing">
    <div aria-hidden="true" :class="cn(
  'slider-value-labels pointer-events-none absolute inset-x-0 top-0 z-[2] h-1/2 text-base',
)" :style="sliderLabelStyles"></div>
    <div class="slider-track pointer-events-none absolute bottom-0 w-full" :style="sliderTrackStyles">
      <div class="slider-indicator absolute top-1/2 z-[2] h-3/4 w-1 -translate-x-1/2 -translate-y-1/2 rounded-sm"></div>
    </div>
  </div>
</template>
